...
def config_menu():
    ...
    app_actions = (
        "sh -c 'TERMINAL=kitty EDITOR=nano kitty -e nano ~/.config/hypr/conf.d/environment_variables.conf'",
        "sh -c 'TERMINAL=kitty EDITOR=nano kitty -e nano ~/.config/hypr/conf.d/keybindings.conf'",
        "sh -c 'TERMINAL=kitty EDITOR=nano kitty -e nano ~/.config/hypr/conf.d/input.conf'",
        "sh -c 'TERMINAL=kitty EDITOR=nano kitty -e nano ~/.config/hypr/conf.d/autostart.conf'",
        "sh -c 'TERMINAL=kitty EDITOR=nano kitty -e nano ~/.config/hypr/conf.d/monitors.conf'",
    ...
    result = subprocess.run(
        ["rofi", "-dmenu", "-p", "Settings...", "-width", "500", "-height", "360"],
        input=options,
        text=True,
...
def system_menu():
    ...
    options = "\n".join(keys)
    result = subprocess.run(
        ["rofi","-dmenu", "-p", "System...", "-width", "500", "-height", "190"],
        input=options,
        text=True,
        stdout=subprocess.PIPE
    ).stdout.strip()
...
def menu_main():
    ...
    options = "\n".join(keys)
    result = subprocess.run(
        ["rofi", "-dmenu", "-p", "Menu...", "-width", "500", "-height", "300"],
        input=options,
        text=True,
        stdout=subprocess.PIPE
    ).stdout.strip()
